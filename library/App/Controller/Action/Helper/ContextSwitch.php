<?php
    class App_Controller_Action_Helper_ContextSwitch extends Zend_Controller_Action_Helper_ContextSwitch {
        protected $_autoSerialization = true;

        protected $_availableAdapters = array (
            'json'  => 'Zend_Serializer_Adapter_Json',
            'xml'   => 'App_Serializer_Adapter_Xml',
        ...
        public function __construct ($options = null) {
            if ($options instanceof Zend_Config) $this->setConfig ($options);
            elseif (is_array ($options)) $this->setOptions ($options);

            if (empty ($this->_contexts)) $this->addContexts ($this->_rest_contexts);
            ...
            if ($this->getAutoSerialization ()) {
                ...
                if ($view instanceof Zend_View_Interface) {
                    if (method_exists ($view, 'getVars')) {
                        $data = $view->getVars ();
                        if (count ($data) !== 0) {
                            if (isset ($data ['titleHead'])) unset ($data ['titleHead']);
