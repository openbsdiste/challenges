...
var QuestionForms = {
    ...
    processTextes: function(a) {
        var c = a.data.id;
        if (document.getElementById("presaisie" + c)) {
            jQuery("#presaisie" + c).append("<br />---<br />");
            jQuery("#presaisie" + c).append(jQuery('#texte' + c).val());
            jQuery("#texte" + c).val("");
            jQuery("#saisie" + c).empty();
        }
...
var GerePassword = {
    setPassword: function() {
        if ((jQuery("#motdepass1").val() != '') && (jQuery("#motdepass1").val() == jQuery("#motdepass2").val())) {
            jQuery.ajax({
                type: "POST",
                datatype: "json",
                url: "restutilisateur/verouillage",
                data: {
                    format: "json",
                    p: jQuery("#motdepass1").val(),
                    t: "definir"
                },
                success: function(d) {
                    Global.showMessage("Mot de passe défini");
                    Tabs.closeAllQuestionsTab();
                    Tabs.closeTab("verouillage");
                },
                error: function(d) {
                    Global.showError(d, "msg_sans_info", "Raté")
                }
            });
        } else {
            Global.showMessage("Les mots de passe doivent correspondre et ne pas être vides");
        }
    },
    checkPassword: function() {
        jQuery.ajax({
            ...
            data: {
                format: "json",
                p: jQuery("#motdepass").val(),
                t: "verifier"
            },
            success: function(d) {
                Global.showMessage("Mot de passe pris en compte");
                Tabs.closeAllQuestionsTab();
                Tabs.closeTab("verouillage");
            },
            error: function(d) {
